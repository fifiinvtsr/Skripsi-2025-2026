"""
Evaluasi Model per Kategori (Tabel 5.7)
Menjalankan pengujian 6 parameter terbaik untuk 3 kategori (Faster, On-time, Over SLA)
Mengekstrak 3 parameter terbaik per kategori dan membuat sheets di hasil_pengujian.xlsx
"""
import pymysql
import pandas as pd
import numpy as np
...
# ========================================
# Output: Print, Ekstrak 1 Parameter Terbaik per Kategori & Export
# ========================================
print(f"\n{'='*80}")
print(f"TOTAL: {len(df_result)} baris (6 parameter x 3 kategori)")
print(f"{'='*80}\n")
print(df_result.to_string(index=False))

# ========================================
# Ekstrak 1 Parameter Terbaik per Kategori (RMSE Terendah)
# ========================================
print(f"\n{'='*80}")
print("1 PARAMETER TERBAIK PER KATEGORI (RMSE Terendah)")
print(f"{'='*80}\n")
...
for kategori in df_result["Kategori"].unique():
    df_kat = df_result[df_result["Kategori"] == kategori].copy()
    df_kat_sorted = df_kat.sort_values("RMSE").head(1).reset_index(drop=True)
    
    hasil_per_kategori[kategori] = df_kat_sorted
    sheets_data[kategori] = df_kat_sorted[["Parameter", "MAPE (%)", "RMSE"]]
    
    print(f"--- {kategori} ---")
    for idx, row in df_kat_sorted.iterrows():
        print(f"  🏆 {row['Parameter']:15s} | MAPE={row['MAPE (%)']:6.2f}% | RMSE={row['RMSE']:8.2f}")
    print()
    
    # Tambah ke combined dengan tambah kolom Kategori
    for idx, row in df_kat_sorted.iterrows():
        all_combined.append({
...
# ========================================
# Export ke hasil_pengujian.xlsx
# ========================================
print(f"{'='*80}")
print("MENYIMPAN KE EXCEL")
print(f"{'='*80}\n")

excel_file = os.path.join(os.path.dirname(__file__), "hasil_pengujian.xlsx")
file_exists = os.path.exists(excel_file)

try:
    with pd.ExcelWriter(excel_file, engine='openpyxl', mode='a' if file_exists else 'w', if_sheet_exists='replace') as writer:
        # Sheet untuk setiap kategori
        for kategori, df_sheet in sheets_data.items():
            sheet_name = f"Evaluasi {kategori}"
            df_sheet.to_excel(writer, sheet_name=sheet_name, index=False)
            print(f"  ✓ Sheet '{sheet_name}' ditambahkan ({len(df_sheet)} rows)")
        
        # Sheet kombinasi semua kategori
        df_combined = pd.DataFrame(all_combined)
        df_combined.to_excel(writer, sheet_name="Evaluasi Semua Kategori", index=False)
        print(f"  ✓ Sheet 'Evaluasi Semua Kategori' ditambahkan ({len(df_combined)} rows)")
    
    print(f"\n✓ File berhasil diperbarui: {excel_file}")
    print(f"  Total sheets baru: 4 (3 kategori + 1 kombinasi)")
    print(f"  Total parameter terbaik ditampilkan: 3 (1 per kategori)")

except Exception as e:
    print(f"✗ Error menyimpan file: {e}")
    # Fallback: simpan ke file terpisah
    fallback_file = os.path.join(os.path.dirname(__file__), "hasil_pengujian_kategori.xlsx")
    ...
    print(f"✓ File fallback disimpan: {fallback_file}")
